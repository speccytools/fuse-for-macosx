		     ...
		     size_t *length )
{
  ...
  if( fd == -1 ) {
    ui_error( "couldn't open '%s': %s", filename, strerror( errno ) );
    return 1;
  }
		      ...
		      size_t length )
{
  ...
  f=fopen( filename, "wb" );
  if(!f) { 
    ui_error( "error opening '%s': %s", filename, strerror( errno ) );
    return 1;
  }
	    
  if( fwrite( buffer, 1, length, f ) != length ) {
    ui_error( "error writing to '%s': %s", filename, strerror( errno ) );
    fclose(f);
    return 1;
  }

  if( fclose( f ) ) {
    ui_error( "error closing '%s': %s", filename, strerror( errno ) );
    return 1;
  }
