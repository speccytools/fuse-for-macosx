...
static size_t compare_memory( void );

int bytes_to_shove = 0;
...
int spectrum_interrupt(void)
{
  ...
  */
  if( rzx_playback ) {
    ...
    if(event_interrupt( machine_current->timings.tstates_per_frame )) return 1;
  ...
  /* 56k bits per second = 7k bytes per second = 140 bytes per frame */
  /* 128k bits per second = 16k bytes per second = 320 bytes per frame */
/*    bytes_to_shove -= 320; */
/*    if( bytes_to_shove < 0 ) { */

/*      bytes_to_shove = compare_memory(); */

/*      printf( "0x%04x (%d frames)\n", bytes_to_shove, */
/*  	    ( bytes_to_shove + 319 ) / 320 ); */
/*    } */
...
static libspectrum_byte oldRAM[8][0x4000];
...
compare_memory( void )
{
  ...
  for( page = 0; page < 8; page++ ) {
    ...
    for( offset = 0; offset < 0x4000; offset++ ) {
      ...
      if( oldRAM[page][offset] != RAM[page][offset] ) {
	
	offset++; length = 1;
	
      find_run:
	for( ;
	     offset < 0x4000 && oldRAM[page][offset] != RAM[page][offset];
	     length++, offset++ ) ;
	
	stored_length = length;

	/* Start looking ahead to see if it's worth merging two runs */
	for( i = 0; i < OVERHEAD; i++ ) {
	  
	  offset++; length++;
	  
	  /* Break out if we've reached the end of a page */
	  if( offset >= 0x4000 ) break;
	  
	  /* If this doesn't match, add to the current run */
	  if( oldRAM[page][offset] != RAM[page][offset] ) goto find_run;
	  
	  /* If it does match, try more bytes until it wouldn't be
	     worth merging the two runs */
	}

	/* When we reach here, it's either because we reached the end
	   of a page or because it wasn't worth adding to the current
	   run */
	size += OVERHEAD + stored_length;
    ...
    if( copy_page ) memcpy( oldRAM[page], RAM[page], 0x4000 );
